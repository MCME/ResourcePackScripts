...
def get_models(data, search_model, filepath):
    ...
    if "variants" in data:
        ...
        result = find_model_entries(data["multipart"], search_model, filepath)
        if result[1]:
            model_found = True
        for rotation in result[0]:
            if rotation not in rotations:
...
def process_directory(directory, search_model):
    ...
    for filepath in directory.rglob("*.json"):  # Alle JSON-Dateien rekursiv durchsuchen
        ...
        if data:
            blockstate_files_and_rotations = find_model_entries(data, search_model, filepath)
    ...
    # print(str(result))
    result = [all_blockstate_files, all_rotations]
    return result
